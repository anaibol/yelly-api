...
async function main() {
  ...
  let postWithEmojisCount = 0
  let emojiCreatedCount = 0

  for (let i = 0; i < posts.length; i += 1) {
    const { id, text, author } = posts[i]
    const countryId = author.school?.city.country.id
    ...
    const uniqEmojis = uniq(emojis)

    for (let j = 0; j < uniqEmojis.length; j += 1) {
  ...
  console.log('result', { emojiCreatedCount })
}
